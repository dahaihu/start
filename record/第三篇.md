...
我想知道这个函数的表现，并且函数给堆上的压力。为了了解这些，我们需要运行下 benchmark。
...
在运行 benchmark 之后，我们可以看到函数`algOne`函数的每次操作都分配了两次内存，并且分配的内存大小为 117 bytes。这个表现非常好了，但是我们需要知道是哪些代码造成了这些内存的分配。为了知道这些，我们需要产生运行 benchmark 的 profiling data。
...
这个说明了，把`bytes.Buffer`的地址传递给函数，并把这个地址作为一个接口存储。这就造成了变量被逃逸到了堆。现在我们看到了使用接口的代价：分配到堆并且间接的使用。如果使用接口并没有使得代码变得更好，那就最好别使用接口。我跟随这下面这些指导来使用接口
...
现在我们需要问自己，这个算法真的需要使用`io.ReadFull`函数吗？答案是否定的，因为`bytes.Buffer`类型有一系列方法可以使用。使用变量已有的方法可以有效的避免变量被分配到堆。

现在我们可以移去`io`包，并使用`input`变量已有的方法`Read`。

下面的代码移去了`io`包，为了保持新的代码和原来的代码行不变，使用了变量`_`来避免导入`io`包。这样，就可以保持`io`包还在引入的行列中。
